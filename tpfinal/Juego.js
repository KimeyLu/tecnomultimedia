class Juego{
  constructor(cantidadDeEnemigos) {
    ...
    this.img = []
      for(let i = 0; i < 8; i++){
    this.img[i] = loadImage('assets/b'+i+'.png')
      } 
    this.cigimg = []
      for(let i = 0; i < 7; i++){
    this.cigimg[i] = loadImage('assets/cigarra'+i+'.png')
      }
    this.estado = 1
    this.contador = 45
    this.crearEnemigos()
  ...
  crearEnemigos(){
    this.enemigos= []
    for( let i = 0; i < this.cantidadDeEnemigos; i++){                                                                                          
      this.enemigos[i]= new Enemigo(i*width/3, random(height/3, height/10), i*width/5, random(height/1.2, height/1.6), width/10, height/15)
    }
  }
